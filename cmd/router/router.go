...
func (r *router) buildCustomersRoutes() {
	...
	c := r.rg.Group("/customers")
	{
		...
		c.GET("/totalCustomerByCondition", handler.TotalCustomerByCondition())
		c.GET("/totalCustomerActiveAmount", handler.TotalAmountActiveCustomers())
		c.POST("", handler.Post())
		c.POST("/loaddata", handler.LoadCustomerData())
	}
}
...
func (r *router) buildInvoicesRoutes() {
	...
	i := r.rg.Group("/invoices")
	{
		...
		i.POST("", handler.Post())
		i.POST("/loaddata", handler.LoadInvoicesData())
		i.PUT("/fixdata", handler.FixInvoicesData())
	}
}
...
func (r *router) buildProductsRoutes() {
	...
	p := r.rg.Group("/products")
	{
		...
		p.GET("/top5Products", handler.Top5Products())
		p.POST("", handler.Post())
		p.POST("/loaddata", handler.LoadProductData())
	}
}
...
func (r *router) buildSalesRoutes() {
	...
	s := r.rg.Group("/sales")
	{
		...
		s.POST("", handler.Post())
		s.POST("/loaddata", handler.LoadSalesData())
	}
}
