...
type Repository interface {
	...
	LoadData(sale *domain.Sales) error
}
...
func (r *repository) LoadData(sale *domain.Sales) error {
	query := `INSERT INTO sales (id, product_id, invoice_id, quantity) VALUES (?, ?, ?, ?)`
	_, err := r.db.Exec(query, &sale.Id, &sale.ProductId, &sale.InvoicesId, &sale.Quantity)
	if err != nil {
		return err
	}
	return nil
